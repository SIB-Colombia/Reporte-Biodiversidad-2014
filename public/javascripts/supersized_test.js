(function($){
	$.fn.supersized=function(){
		...
		$(window).bind("load",function(){
			$('#loading').hide();
			$('#supersize').fadeIn('fast');$('#content').show();
			...
			if(options.slideshow==1){
				if(options.slide_counter==1){$('#slidecounter .slidenumber').html(1);
				$('#slidecounter .totalslides').html($("#supersize > *").size());
			}
			console.log("TIME: "+options.slide_interval);
			slideshow_interval=setInterval("nextslide()",options.slide_interval);
			if(options.navigation==1){
				$('#navigation a').click(function(){$(this).blur();return false;});
				$('#nextslide').click(function(){
					if($.paused)return false;
					if($.inAnimation)return false;clearInterval(slideshow_interval);
					nextslide();slideshow_interval=setInterval(nextslide,options.slide_interval);
					return false;
				});
				...
				$('#nextslide img').hover(function(){
					...
					$(this).attr("src","images/forward.gif");},function(){if($.paused==true)return false;$(this).attr("src","images/forward_dull.gif");});$('#prevslide img').hover(function(){if($.paused==true)return false;$(this).attr("src","images/back.gif");},function(){if($.paused==true)return false;$(this).attr("src","images/back_dull.gif");});$('#pauseplay').click(function(){if($.inAnimation)return false;var src=($(this).find('img').attr("src")==="images/play.gif")?"images/pause.gif":"images/play.gif";if(src=="images/pause.gif"){$(this).find('img').attr("src","images/play.gif");$.paused=false;slideshow_interval=setInterval(nextslide,options.slide_interval);}else{$(this).find('img').attr("src","images/pause.gif");clearInterval(slideshow_interval);$.paused=true;}
...
return false;});}}});$(document).ready(function(){});$('#supersize > *').hover(function(){if(options.slideshow==1&&options.pause_hover==1){if(!($.paused)&&options.navigation==1){$('#pauseplay > img').attr("src","images/pause.gif");clearInterval(slideshow_interval);}}
